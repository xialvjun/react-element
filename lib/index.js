...
exports.init_value = function (value) { return function (ele) {
    ...
    ele.set = ele.set_partial = function (path, path_value, callback) { return ele.set_value(js_utils_1.set_path(ele.value, path, path_value), callback); };
    ele.get = ele.get_partial = function (path) { return js_utils_1.get_path(ele.value, path); };
    ele.merge = ele.set_state = function (value, callback) { return ele.set_value(Object.assign({}, ele.value, value), callback); };
}; };
exports.init_state = function (state) { return function (ele) {
    ...
    ele.set_partial = function (path, path_value, callback) { return ele.set_state(js_utils_1.set_path(ele.state, path, path_value), callback); };
}; };
exports.init_ref = function (name) { return function (ele) {
    name = name || "ref";
    ele[name] = React.createRef();
}; };
exports.init_refs = function (name, use_create_ref) {
    if (use_create_ref === void 0) { use_create_ref = true; }
    return function (ele) {
        ele[name] = new Proxy({}, {
            get: function (obj, prop) {
                if (!obj[prop]) {
                    var ref_1 = obj[prop] = use_create_ref ? React.createRef() : function (e) { return ref_1.current = e; };
                }
                return obj[prop];
            }
        });
